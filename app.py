...
def load_and_prepare_data(path: str) -> pd.DataFrame:
    ...
    risk_score = (
        ...
        + df["Saving accounts"]
        .map({"little": 1.5, "moderate": 0.8, "quite rich": 0.4, "rich": 0.2})
        .fillna(1.0)
        + df["Checking account"]
        .map({"little": 1.2, "moderate": 0.6, "rich": 0.2})
        .fillna(0.9)
...
_DEFAULT_CSV = os.path.join(_SCRIPT_DIR, "german_credit_data.csv")

if not os.path.exists(_DEFAULT_CSV):
    st.error(
        f"Dataset not found: `{_DEFAULT_CSV}`\n\n"
        "Please place **german_credit_data.csv** in the same folder as this script."
    )
    st.stop()

df = load_and_prepare_data(_DEFAULT_CSV)
...
with st.sidebar:
    ...
    threshold = st.slider(
        "Probability Threshold", 0.1, 0.9, 0.5, 0.05,
...
st.markdown(
    '<p class="sub-header">ML-powered credit risk evaluation using the German Credit Dataset</p>',
    unsafe_allow_html=True,
)

tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Dashboard", "Model Performance", "Feature Analysis", "Predict Risk", "Data Explorer"]
)
...
with tab1:
    ...
    with col_r:
        fig = px.box(
            df, x="Risk_Label", y="Credit amount",
            color="Risk_Label",
            color_discrete_map={"Good": "#2e7d32", "Bad": "#e53935"},
    ...
    with col_l2:
        fig = px.histogram(
            df, x="Age", color="Risk_Label", nbins=25,
    ...
    with col_r2:
        ...
        fig = px.bar(
            purpose_risk, x="Purpose", y="Count", color="Risk_Label",
            barmode="stack",
            color_discrete_map={"Good": "#2e7d32", "Bad": "#e53935"},
    ...
    st.dataframe(
        model_summary.style.highlight_max(subset=["Accuracy"], color="#d4edda"),
...
with tab2:
    st.markdown(
        f'<p class="section-title">{selected_model_name} — Detailed Performance</p>',
        unsafe_allow_html=True,
    )
...
with tab3:
    ...
    with col_l:
        if hasattr(model, "feature_importances_"):
            ...
            fig = px.bar(
                feat_df, x="Coefficient (|abs|)", y="Feature", orientation="h",
            ...
            st.info("Feature importance not directly available for this model. Use Permutation Importance (right panel).")
    ...
    fig = px.scatter(
        df, x="Duration", y="Credit amount", color="Risk_Label",
...
with tab4:
    st.markdown(
        '<p class="section-title">Predict Credit Risk for New Applicant</p>',
        unsafe_allow_html=True,
    )
    ...
    if submitted:
        ...
        purpose_list = ["car", "domestic appliances", "education", "furniture/equipment",
                        "business", "radio/TV", "repairs", "vacation/others"]
        purpose_enc = purpose_list.index(purpose) if purpose in purpose_list else 0
        ...
        with res_col1:
            if prediction == 1:
                st.markdown(
                    f'<div class="risk-high">HIGH RISK — BAD CREDIT<br>Probability: {prob_bad:.1%}</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<div class="risk-low">LOW RISK — GOOD CREDIT<br>Probability of Bad: {prob_bad:.1%}</div>',
                    unsafe_allow_html=True,
                )
        ...
        similar = df[
            ...
            ["Age", "Sex", "Housing", "Saving accounts", "Checking account",
             "Credit amount", "Duration", "Purpose", "Risk_Label"]
        ].head(10)
...
with tab5:
    ...
    st.dataframe(
        filtered[["Age", "Credit amount", "Duration", "Job"]].describe().T,
        use_container_width=True,
    )
